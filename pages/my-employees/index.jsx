...
const MyEmployees = (props) => {
    ...
    const calcExpenses = (expensesArr) => {
        ...
        expensesArr?.forEach((day) => {
            totalExpenses += (parseFloat(day.fuel) + parseFloat(day.salary) + parseFloat(day.other))
        });
        return (
            totalExpenses
        )
    ...
    return (
        <div className={styles.myEmployeesWrapper}>
            ...
            <div className={styles.myEmployeesContainer}>
                <AddEmployeeForm userData={props?.userData} handleUserData={props?.handleUserData} />
                <div className={styles.employees}>
                    {employees?.map((employee) => {
                        return (
                            <div className={styles.employee} key={employee?.idNumber}>
                                <div className={styles.imagesWrapper}>
                                    <Image src={placeHolderProfileImage} height={150} width={150} alt={`${employee?.firstname} ${employee?.lastname} profile image`} />
                                    <Image src={placeHolderCarImage} height={150} width={150} alt={`car image`} />
                                </div>
                                <div className={styles.employeeDetails}>
                                    <h4>שם העובד: {employee?.firstname} {employee?.lastname}</h4>
                                    <h5>דרגה: {employee?.roleHe}</h5>
                                    <h5>רכב מקושר: {employee?.carAttached}</h5>
                                    <h5>טלפון: {employee?.phoneNumber}</h5>
                                    <h5>אימייל: {employee?.email}</h5>
                                    <h5>דרגת רישיון: {employee?.licenseLevel}</h5>
                                    <h5>סך כל ההכנסות: {
                                        calcIncome(employee?.income).toFixed(2)
                                    } ₪</h5>
                                    <h5>הוצאות: {
                                        calcExpenses(employee?.expenses).toFixed(2)
                                    } ₪</h5>
                                    <h5>רווח:  {(calcIncome(employee?.income) - calcExpenses(employee?.expenses)).toFixed(2)} ₪</h5>
                                    <h5>תחילת עבודה: {employee?.jobStarted}</h5>
                                    <h5>תחילת עבודה: {employee?.jobEnded ? employee?.jobEnded : "עובד פעיל"}</h5>
                                    <h5>חופשה מתוכננת : {employee?.vacationMode?.vacation ? 'כן' : 'לא'} {employee?.vacationMode?.vacation && <>
                                        <br />
                                        <span>מתאריך: {employee?.vacationMode?.fromDate} לתאריך : {employee?.vacationMode?.toDate}</span>
                                    </>}
                                    </h5>
                                </div>
                            </div>
