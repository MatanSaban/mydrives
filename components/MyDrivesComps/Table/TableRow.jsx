        ...
        <div className={`${styles.driveItem} driveItem`}>
            ...
            <div id={props.item.id} className={`${styles.row} row`}>
                <div className={styles.right}>
                    ...
                    <div data={"startTime"} className={styles.data_row}>
                        <strong>התחלת נסיעה:</strong>
                        <span> {props.item.startTime}</span>
                    </div>
                    <div
                        data={"estimatedDrivingTime"}
                        className={styles.data_row}
                    >
                        <strong>זמן מוערך:</strong>
                        <span> {props.item?.estimatedDrivingTime}</span>
                    </div>
                    <div data={"endTime"} className={styles.data_row}>
                        <strong>סיום מוגדר:</strong>
                        <span> {props.item.endTime}</span>
                    </div>
                    <div data={"startPoint"} className={styles.data_row}>
                        <strong>נק&apos; התחלה:</strong>
                        <span> {props.item?.startPoint}</span>
                    </div>
                    {props.item?.stops?.length > 0 && (
                        <div className={`${styles.data_row} ${styles.stops}`}>
                            <strong>מס&apos; עצירות: </strong>
                            <span>{props?.item?.stops?.length} עצירות</span>
                            <button
                                onClick={() =>
                                    props.handlePopup(
                                        true,
                                        print_stops(props?.item?.stops)
                                    )
                                }
                            >
                                הצג עצירות
                    ...
                    <div data={"endPoint"} className={styles.data_row}>
                        <strong>נק&apos; סיום:</strong>
                        <span> {props.item?.endPoint}</span>
                    </div>
                </div>
                <div className={styles.left}>
                    <div data={"date"} className={styles.data_row}>
                        <strong>תאריך: </strong>
                        <span>{props.dateFormatter(props.item.date)}</span>
                    </div>
                    <div data={"fuelPrice"} className={styles.data_row}>
                        <strong>דלק לליטר: </strong>
                        <span>
                            {parseFloat(props.item.fuelPrice).toFixed(2)} ₪
                        </span>
                    </div>
                    <div data={"kilometers"} className={styles.data_row}>
                        <strong>קילומטרים: </strong>
                        <span>
                            {parseFloat(props.item.kilometers).toFixed(2)}{" "}
                            ק&quot;מ
                        </span>
                    </div>
                    <div data={"fuelConsumption"} className={styles.data_row}>
                        <strong>צריכת דלק: </strong>
                        <span>
                            {(
                    ...
                    <div
                        className={`${styles.priceAndProfit} ${styles.data_row}`}
                    >
                        <div data={"price"}>
                            <strong>מחיר: </strong>
