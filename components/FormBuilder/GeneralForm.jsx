...
const GeneralForm = (props) => {
  ...
  const onFocusFunc = (e) => {
    ...
    if (e.target.attributes["dateandtext"]?.value === "true") {
      setDateInputsFocused((prevState) => ({
        ...prevState,
        [e.target.name]: true,
      }));
    }
  ...
  const onLabelClickFunc = (e) => {
    ...
    if (
      !labelWrapper.classList.contains(styles.filled) &&
      !labelWrapper.classList.contains(styles.focused)
    ) {
      labelWrapper.classList.add(styles.focused);
      setFocusedLabel(labelWrapper.id);
    }
          ...
          <div
            className={`${styles.labelWrapper} ${
              field?.defaultValue?.length > 0
                ? `${styles.filled} ${styles.inPopupForm}`
                : ``
            } labelWrapper`}
            onClick={(e) => onLabelClickFunc(e)}
            key={field.name}
          >
            {console.log("field")}
            {console.log(field)}
            <label
              className={`${styles.labelWrapper} ${
                field?.defaultValue?.length > 0 &&
                props?.parentPage != "myaccountpage"
                  ? styles.inPopupForm
                  : ""
              }`}
            >
            ...
            {field.type === "select" && (
              <select
                ...
                defaultValue={
                  field?.defaultValue && checkForPassedData(field?.defaultValue)
                }
            ...
            {field.type !== "select" && (
              <input
                type={
                  dateInputsFocused?.[field.name] === true ? "date" : "text"
                }
                name={field.name}
                onChange={(e) => onChangeFunc(e)}
                onBlur={(e) => onBlurFunc(e)}
